...
class Trader():
    ...
    def close(self, volume, direction, price=None, orderType='opponent'):
        if price:
            price = round(price, 2)
    ...
    def buy(self, volume, price=None, orderType='opponent'):
        if price:
            price = round(price, 2)
        result = self.dm.send_contract_order(
            symbol=self.symbol, contract_type=self.contract_type,
            contract_code='', client_order_id='', price=price,
            volume=volume,
    ...
    def sell(self, volume, price=None, orderType='opponent'):
        if price:
            price = round(price, 2)
        result = self.dm.send_contract_order(
            symbol=self.symbol, contract_type=self.contract_type,
        ...
        if result['status'] == 'error':
            ...
            self.orderId = result['data']['order_id']
            tracker = Tracker(self.dm, self.rderId, self.callback)
            tracker.start()
