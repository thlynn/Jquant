...
class FutureAPI(Client):
    ...
    # take order
    def take_order(self, instrument_id, otype, price, size, match_price, leverage, client_oid=None):

        params = {'instrument_id': instrument_id, 'type': otype, 'price': price, 'size': size, 'match_price':match_price, 'leverage': leverage}
        if client_oid:
